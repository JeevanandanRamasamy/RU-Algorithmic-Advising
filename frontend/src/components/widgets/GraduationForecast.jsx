/**
 * GraduationForecastCard Component
 *
 * This component estimates how many semesters a student needs to graduate based on:
 * - Total credits required
 * - Credits already earned
 * - Assumed number of credits per semester (adjustable)
 *
 * It dynamically displays:
 * - Number of semesters remaining
 * - Custom motivational/progress messages based on progress
 *
 * Props:
 * - totalCredits (number or string): Total number of credits needed to graduate
 * - creditsEarned (number or string): Credits already completed by the student
 *
 * Features:
 * - Input to adjust planned credits per semester (default: 15)
 * - Real-time forecast updates and feedback
 * - Handles boundary conditions and realistic minimum/maximum semester loads
 */
...
const GraduationForecastCard = ({ totalCredits, creditsEarned }) => {
  ...
  if (earned === 0.0) {
    progressMessage =
      "Let’s get started! Begin enrolling in courses to make progress. 🚀";
    ...
    progressMessage =
      "Consider increasing your credits per semester to stay on track. ⏳";
  } else if (semestersLeft > 6) {
    progressMessage =
      "You might need to take more credits or stay longer to graduate on time. 🚨";
  } else {
