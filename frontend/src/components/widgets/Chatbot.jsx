/**
 * Chatbot & ChatPanel Components
 *
 * This file implements an AI-powered floating chatbot interface using React and Framer Motion.
 * It allows users to open a chat window, send messages, and receive AI responses (via Gemini).
 *
 * Components:
 * - Chatbot: Handles toggling the chat panel and loading user info.
 * - ChatPanel: The visual UI for the chatbot with chat bubbles, input field, send button, and loading animation.
 *
 * Key Features:
 * - Framer Motion for entrance/exit animations and button effects
 * - React Markdown to render AI-generated markdown responses
 * - Auto-scrolls when new messages arrive
 * - Input with "Enter to send" support
 * - Powered by `useAIChat` hook for managing state and communication
 */
  ...
  return (
    ...
    >
      <div className="p-4 border-b bg-[#cc0033] rounded-t-xl flex items-center gap-2">
        ...
        <span className="font-semibold text-[#fcf8d7]">
          Algorithmic Advisor
        </span>
      </div>
      ...
      <div className="flex-1 p-3 overflow-y-auto space-y-3 text-sm">
        ...
        {messages.map((msg, i) => (
          <div
            key={i}
            className={`max-w-[75%] px-3 py-1 rounded-2xl shadow-sm ${
              msg.sender === "user"
      ...
      <div className="text-center text-xs text-gray-400 p-1">
        ...
        <a
          ...
          className="text-[#cc0033] hover:underline"
        >
          Gemini
        </a>
      </div>
...
const Chatbot = () => {
  ...
  const { messages, input, setInput, sendMessage, loading, setUserInfo } =
    useAIChat();
  ...
  return (
    <>
      <motion.button
        ...
        whileTap={{ scale: 0.9 }} // Scale tap (press animation)
