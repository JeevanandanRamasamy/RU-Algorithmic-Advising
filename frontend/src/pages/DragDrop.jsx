...
function DragDrop() {
  ...
  const fetchPlannedCourses = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/planned_courses`, {
        headers: {
  ...
  const addCourseToPlan = async (courseId) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/planned_courses/add', {
        method: 'POST',
        headers: {
  ...
  const dropCourseFromPlan = async (courseId) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/planned_courses/drop', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
    ...
    <div className="app">
      ...
      <main className="course-planner">
        <section className="available-courses">
          ...
          <CourseList courses={courses} isPlanned={false} />
        </section>
