...
class UserPlan:
    ...
    def get_planned_courses_for_user():
        ...
        try:
            ...
            degree_plans = DegreePlan.query.filter_by(username=username).all()  # Get the user's degree plans
            ...
            for plan in degree_plans:
                courses = PlannedCourse.query.filter_by(plan_id=plan.plan_id).all()  # Get planned courses for each degree plan
    ...
    def add_course_to_plan(course_id, plan_id, term, year):
        ...
        if not degree_plans:
            ...
            degree_plan = degree_plans[0]  # Assuming user can have multiple plans, select the first one.
    ...
    def drop_course_from_plan(plan_id, course_id):
        ...
        degree_plan = next((plan for plan in degree_plans if plan.plan_id == plan_id), None)
