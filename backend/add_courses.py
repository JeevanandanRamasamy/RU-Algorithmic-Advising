...
def parse_and_simplify_prereqs(subject, pre_reqs):
    ...
    if prereq_str.endswith('course within the subject area:'):
        ...
        prereq_str = prereq_str.replace('one', '1').replace('two', '2').replace('three', '3').replace('four', '4').replace('five', '5').replace('six', '6').replace('seven', '7').replace('eight', '8').replace('nine', '9').replace('ten', '10')
        return {"logic": f"ATLEAST {prereq_str}", "courses": [f"XX:{subject}:XXX"]}
    if prereq_str.find('course from the following:') != -1: # if the prereq is a list of courses
        number = prereq_str.split(' ')[1]
        number = number.replace('one', '1').replace('two', '2').replace('three', '3').replace('four', '4').replace('five', '5').replace('six', '6').replace('seven', '7').replace('eight', '8').replace('nine', '9').replace('ten', '10')
    ...
    course_symbols = {course: symbols(f"C{idx}") for idx, course in enumerate(course_ids)}
    ...
    simplified_expr = simplify_logic(boolean_expr, form='dnf')  # Disjunctive Normal Form
...
def parse_courses(json_data, courses):
    ...
    for course in json_data:
        course_id = course['offeringUnitCode'] + ':' + course['subject'] + ':' + course['courseNumber']
        ...
        prerequisites = parse_and_simplify_prereqs(course['subject'], course.get('preReqNotes', None))
...
def get_course_list():
    ...
    semesters = [f'{month}{year}' for year in ['2025', '2024', '2023', '2022'] for month in ['9', '7', '1',]]
