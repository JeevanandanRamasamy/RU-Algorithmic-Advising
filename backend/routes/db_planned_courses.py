...
from services.user_plan_service import UserPlan  # Import the service layer for planned course-related operations
...
def get_planned_courses():
    try:
        ...
        planned_courses = UserPlan.get_planned_courses_for_user()

        if isinstance(planned_courses, list):  # Check if the return value is a valid list
            response_data = {
                "plan_id": planned_courses[0].get('plan_id') if planned_courses else None,
                "courses": planned_courses
            }
            return jsonify(response_data), 200
        else:
            return jsonify({"message": planned_courses}), 500  # Return error message if something went wrong
...
def add_planned_course():
    ...
    try:
        ...
        response = UserPlan.add_course_to_plan(course_id, plan_id, term, year)
...
def drop_planned_course():
    if request.method == 'OPTIONS':
        ...
        try:
            ...
            response = UserPlan.drop_course_from_plan(plan_id, course_id)
