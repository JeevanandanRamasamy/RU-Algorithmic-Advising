...
class Account(db.Model):
    ...
    role = Column(Enum('student', 'admin', name='role_enum'), default='student', nullable=False)
...
class StudentDetails(db.Model):
    ...
    def __repr__(self):
        return f"<StudentDetails(username={self.username}, class_year={self.class_year})>"
...
class CourseTaken(db.Model):
    ...
    course_id = Column(String(10), ForeignKey('Course.course_id', ondelete='CASCADE'), nullable=False)
    term = Column(Enum('fall', 'spring', 'summer', 'winter', name='term_enum'))
    year = Column(YEAR)
    grade = Column(String(2), CheckConstraint("grade IN ('A', 'B+', 'B', 'C+', 'C', 'D', 'F', 'PA', 'NC', 'W')"))

    __table_args__ = (
        PrimaryKeyConstraint('username', 'course_id', 'term', 'year'),
    )
...
class Program(db.Model):
    ...
    program_type = Column(Enum('major', 'minor', 'certificate', 'sas_core', name='program_type_enum'), nullable=False)
...
class StudentProgram(db.Model):
    ...
    username = Column(String(6), ForeignKey('StudentDetails.username', ondelete='CASCADE'), nullable=False)
    program_id = Column(String(7), ForeignKey('Program.program_id', ondelete='CASCADE'), nullable=False)

    __table_args__ = (
        PrimaryKeyConstraint('username', 'program_id'),
    )

    def __repr__(self):
        return f"<StudentProgram(username={self.username}, program_id={self.program_id})>"
...
class PlannedCourse(db.Model):
    ...
    plan_id = Column(Integer, ForeignKey('DegreePlan.plan_id', ondelete='CASCADE'), nullable=False)
    course_id = Column(CHAR(10), ForeignKey('Course.course_id', ondelete='CASCADE'), nullable=False)
    term = Column(Enum('fall', 'spring', 'summer', 'winter', name='term_enum'), nullable=False)
    year = Column(YEAR, nullable=False)

    __table_args__ = (
        db.PrimaryKeyConstraint('plan_id', 'course_id'),
    )
...
class SchedulePlan(db.Model):
    ...
    def __repr__(self):
        return (f"<SchedulePlan(schedule_id={self.schedule_id}, username={self.username}, schedule_name={self.schedule_name}, "
                f"last_updated={self.last_updated}, term={self.term}, year={self.year})>")

class Section(db.Model):
    __tablename__ = 'Section'

    schedule_id = Column(Integer, ForeignKey('SchedulePlan.schedule_id', ondelete='CASCADE'), nullable=False)
    course_id = Column(CHAR(10), ForeignKey('Course.course_id', ondelete='CASCADE'), nullable=False)
    ...
    def __repr__(self):
        return (f"<Section(schedule_id={self.schedule_id}, course_id={self.course_id}, section_num={self.section_num}, "
