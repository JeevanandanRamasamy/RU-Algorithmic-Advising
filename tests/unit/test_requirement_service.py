...
PREREQUISITES_FILE_PATH = os.path.join(
    os.path.join(os.path.dirname(__file__), "..", "..", "backend", "data", "prerequisites.json")
)
...
def test_get_all_prerequisites(client):
    with client.application.app_context():
        ...
        assert prerequisites == {'21:640:113', '21:640:114', '50:640:113', '50:640:114', '01:640:112', '01:640:115'}
...
def test_check_requirements_met(client, register_user, add_courses_records):
    with client.application.app_context():
        for program_id in PROGRAMS:
            ...
            assert requirements_met is False, f"Requirements should not be met for program {program_id}"
        
        extra_courses = ["01:198:205", "01:198:206", "01:198:211", "01:198:344", "01:640:151", "01:640:152", 
                         "01:640:250", "01:198:210", "01:198:213", "01:198:214", "01:198:314", "01:198:336", 
                         "01:198:439", "01:198:462", "01:750:203", "01:750:204", "01:750:205", "01:750:206"]
        requirements_met = RequirementService.check_requirements_met(
        ...
        for course_id in ["01:198:211", "01:198:205", "01:640:151"]:
            ...
            assert isinstance(requirements_met, bool)
            assert requirements_met is True, f"Requirements should be met for course {course_id}"
...
def test_check_group_fulfillment(client):
    with client.application.app_context():
        ...
        assert group_47_fulfilled is False, "Group 47 should not be fulfilled with current courses"
        ...
        assert group_48_fulfilled is True, "Group 48 should be fulfilled with current courses"
...
def test_get_num_requirements(client):
    with client.application.app_context():
        ...
        for program_id, num_reqs in zip(PROGRAMS, num_requirements):
            ...
            assert result == num_reqs, f"Number of requirements for program {program_id} should be {num_reqs}"
        ...
        for course_id, num_reqs in zip(courses, num_requirements):
            ...
            assert isinstance(result, int)
            assert result == num_reqs, f"Number of requirements for course {course_id} should be {num_reqs}"
...
def test_get_num_courses_taken(client, register_user, add_courses_records):
    with client.application.app_context():
        ...
        for program_id, num_courses in zip(PROGRAMS, num_courses_taken):
            ...
            print(f"Number of courses taken for program {program_id}: {num_courses_taken}")
            assert isinstance(num_courses_taken, int)
            assert num_courses_taken == num_courses, f"Number of courses taken for program {program_id} should be {num_courses}"
        ...
        for course_id, num_courses in zip(courses, num_courses_taken):
            ...
            assert isinstance(num_courses_taken, int)
            assert num_courses_taken == num_courses, f"Number of courses taken for course {course_id} should be {num_courses}"
...
def test_get_suggested_courses(client, register_user, add_courses_records):
    with client.application.app_context():
        ...
        for course in COURSES_TAKEN:
            assert course not in suggested_courses, f"Course {course} should not be in suggested courses"
...
def test_create_degree_plan(client, register_user, add_courses_records):
    with client.application.app_context():
        ...
        for planned_set in degree_plan:
            assert isinstance(planned_set, set), "Each entry in degree_plan should be a set"
            assert taken_set.isdisjoint(planned_set), \
